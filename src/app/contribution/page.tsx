...
const ContributionPage = () => {
  ...
  const updateContribution = async () => {
    if (!contributionId) {
      setResponse(
        '<p class="text-red-500">Please enter a valid Contribution ID.</p>'
      );
      return;
    }
    ...
    try {
      const res = await fetch(
        `http://localhost:8010/proxy/api/datacuration/v1/contribution/update?id=${contributionId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!res.ok) {
        setResponse(
          `<p class="text-red-500">Error: ${
            data.error || "Could not update the contribution."
          }</p>`
        );
        return;
      }
      ...
      setResponse(
        `<p class="text-green-500">Contribution updated successfully:</p><pre>${JSON.stringify(
          data,
          null,
          2
        )}</pre>`
  ...
  const batchUpdateContributions = async () => {
    if (batchStopped) {
    ...
    try {
      const res = await fetch(
        `http://localhost:8010/proxy/api/datacuration/v1/contributions/clean`,
        {
          method: "GET",
        }
      );
      ...
      if (!res.ok) {
        setBatchResponse(
          `<p class="text-red-500">Error: ${
            data.error || "Could not batch update the contributions."
          }</p>`
        );
        return;
      }
    ...
    <div className="flex flex-col items-center min-h-screen bg-background text-foreground">
      ...
      <div className="flex flex-col md:flex-row justify-center items-start mt-8 w-full px-4 md:px-0">
        ...
        <div className="bg-card p-6 m-4 rounded-lg shadow-md w-full md:w-1/2">
          <h1 className="text-xl font-semibold mb-4">Curate Contribution</h1>
          <label htmlFor="contributionId" className="block mb-2 font-medium">
            Enter Contribution ID:
          </label>
          <input
            type="text"
            id="contributionId"
        ...
        <div className="bg-card p-6 m-4 rounded-lg shadow-md w-full md:w-1/2">
          ...
          <div className="flex space-x-4 mt-4">
            <button
              onClick={batchUpdateContributions}
